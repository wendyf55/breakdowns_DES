...
Current matching flags cross-platform name mismatches but does not yet expand a
query name to its accepted-name/synonym set. Keep genus blocking for now, but add
synonym evidence before relaxing match thresholds:
...
- Build a cached `data/name_synonyms.csv` from the MDS synonym API pipeline
  ...
  `scripts/name_synonyms.py` is the first local scaffold for this cache.
- Store `query_name`, `accepted_name`, `synonym_name`, `source`, source id/link,
  and fetch timestamp.
